...
- 新規 task はまず project に作成し、section 指定がある場合は作成後に section へ配置する
...
- `ASANA_COMPACT_STATUS_SECTIONS`
  `ASANA_USE_STATUS_SECTIONS` 有効時のみ使用。未設定時のデフォルトは `true`。
  `実装中` / `実装着手済み` / `Phase 1実装中` を `着手中`、`MVP実装済み` / `MVP実装完了` / `実装済み` を `実装完了`、`分割済み` を `分離済み`、`手動ローンチ実行待ち` を `手動待ち` に寄せ、Asana section の増えすぎを防ぐ。
  source の状態名をそのまま section にしたい場合は `false` / `0` / `no` / `off` を指定する。
...
- `ASANA_RECONCILE_REMOVAL_LIMIT`
  実同期時に、source 側から消えた managed task と重複 managed task の削除候補合計がこの数を超えたら同期を止める。デフォルト `20`。
  大量削除が意図したものだと dry-run で確認できた場合だけ、この値を一時的に引き上げる。
...
- `ASANA_COMPACT_STATUS_SECTIONS` も未設定では有効（default true）になり、類似 status を少数の運用 section に寄せる
- 状態が空の場合は `未分類` section に入る
- status 文字列に `・` `/` `｜` `|` が含まれる場合は先頭トークンを代表ラベルとして section 化する（例: `実装完了・デプロイ待ち` → `実装完了`）
- `ASANA_STATUS_SECTION_MAP_JSON` は「生の status」「代表ラベル」「compact 後の section 名」の順で照合し、該当があればその section 名を優先する
...
Asana 認証情報がない dry-run では既存 task を検査できないため、`_taskAction` / `_sectionAction` は `not-inspected` になります。この場合は source 読込と section 名の確認だけに使い、作成 / 更新 / 移動件数の判断には使わないでください。
...
削除候補が `ASANA_RECONCILE_REMOVAL_LIMIT` を超える場合は実同期を止めるため、source 側の一時的な異常や大規模な行削除で入口一覧が一気に崩れるリスクを抑えます。
